...
from app.services.providers.etsy import EtsyProvider
...
class ProviderStatusService:
    ...
    async def test_provider(self, provider: ProviderId) -> ProviderTestResponse:
        start = perf_counter()
        ...
        try:
            ...
            if provider == "youtube":
                return self._with_timing(await self._test_youtube(checked_at), start)
            if provider == "etsy":
                return self._with_timing(await self._test_etsy(checked_at), start)
            if provider == "un_comtrade":
                return self._with_timing(await self._test_un_comtrade(checked_at), start)
    ...
    def _status_for(self, provider: ProviderId) -> ProviderCapabilityStatus:
        ...
        if provider == "etsy":
            if not self._settings.enable_etsy:
                return "disabled"
            return "configured" if self._settings.etsy_keystring else "not_configured"
        if provider == "un_comtrade":
    ...
    async def _test_youtube(self, checked_at: datetime) -> ProviderTestResponse:
        payload = await self._youtube_provider.search_videos("home decor", country="US", max_results=1)
        sample_count = len(payload.items)
        return self._provider_payload_response(
            provider="youtube",
            checked_at=checked_at,
    ...
    async def _test_etsy(self, checked_at: datetime) -> ProviderTestResponse:
    ...
    async def _test_un_comtrade(self, checked_at: datetime) -> ProviderTestResponse:
